...
object PASSProcessModelParserPlugin extends AutoPlugin {
  private val logger = LoggerFactory.getLogger(PASSProcessModelParserPlugin.getClass)
  ...
  object autoImport {
    lazy val parsePASSClass = settingKey[String]("full main class name that parses PASS Process Model files")
  ...
  lazy val basePASSProcessModelParserSettings: Seq[Setting[_]] = Def.settings(
  ...
  private def logParsing(logger: ManagedLogger, args: Seq[String]): Unit = {
    ...
    logger.info(s"Parsing ${sources.length} PASS Process Model sources to $typ in $outDir ...")
  }

  override lazy val projectSettings: Seq[Setting[_]] = basePASSProcessModelParserSettings

  private object ParsePASSProcessModels {
