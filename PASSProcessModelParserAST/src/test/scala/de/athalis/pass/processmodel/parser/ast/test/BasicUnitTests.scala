...
class BasicUnitTests extends AnyFunSuite with Matchers {
  ...
  test("mapTest") {
    parse(PASSParser.mapParserAny, "{->}")                        shouldBe (MAPNode.empty)
    parse(PASSParser.mapParserAny, "{a -> b}")                    shouldBe (MAPNode(Map("a" -> "b")))
    parse(PASSParser.mapParserAny, "{\"a\" -> \"b\"}")            shouldBe (MAPNode(Map("a" -> "b")))
    parse(PASSParser.mapParserAny, "{a -> b, c -> d}")            shouldBe (MAPNode(Map("a" -> "b", "c" -> "d")))
    parse(PASSParser.mapParserAny, "{1 -> [1,2], x -> {a -> b}}") shouldBe (MAPNode(Map(1 -> Vector(1,2), "x" -> Map("a" -> "b"))))
    parse(PASSParser.mapParser(PASSParser.ID, PASSParser.ID), "{a -> b}") shouldBe (MAPNode(Map("a" -> "b")))
    parse(PASSParser.mapParser(PASSParser.NUMBER, PASSParser.ID), "{1 -> b}") shouldBe (MAPNode(Map(1 -> "b")))
    parse(PASSParser.mapParser(PASSParser.ID, PASSParser.listParser(PASSParser.NUMBER)), "{a -> [1,2]}") shouldBe (MAPNode(Map("a" -> Seq(1,2))))
    parse(PASSParser.mapParser(PASSParser.NUMBER, PASSParser.ID), "{1 -> b}") shouldBe (MAPNode(Map(1 -> "b")))
  }

  test("mapTestInvalidKey") {
    an[ParserException] shouldBe thrownBy {
      parse(PASSParser.mapParser(PASSParser.idOrStringParser, PASSParser.mapAbleParser), "{1 -> b}")
    }
  }

  test("mapTestInvalidValue") {
    an[ParserException] shouldBe thrownBy {
      parse(PASSParser.mapParser(PASSParser.mapAbleParser, PASSParser.idOrStringParser), "{a -> [1,2]}")
    }
  }
}
