...
import de.athalis.pass.processmodel.parser.ast.util._
...
object PASSParser {
  ...
  def listParser[T](parser: Parser[MapAbleNode[T]]): Parser[MapAbleNode[Seq[T]]] = parser.sepBy(comma).asSeqNode.between(lParenSqr, rParenSqr)
  def setParser[T](parser: Parser[MapAbleNode[T]]): Parser[MapAbleNode[Set[T]]] = parser.sepBy(comma).asSetNode.between(lParenCurl, rParenCurl)

  val listParserAny: Parser[MapAbleNode[Seq[Any]]] = listParser(MapAbleRef.`lazy`())
  val setParserAny: Parser[MapAbleNode[Set[Any]]] = setParser(MapAbleRef.`lazy`())

  def mapParser[K, V](keyParser: Parser[MapAbleNode[K]], valueParser: Parser[MapAbleNode[V]]): Parser[MapAbleNode[Map[K, V]]] = Parsers.or(Seq(
    kw("->").between(lParenCurl, rParenCurl).map[MapAbleNode[Map[K, V]]](MAPNode.PARSEREmpty[K, V]()),
    {
      val pairParser: Parser[(MapAbleNode[K], MapAbleNode[V])] = ParserUtils.parsePair[MapAbleNode[K], MapAbleNode[V]](keyParser.followedBy(kw("->")), valueParser)
      val mapParser: Parser[MapAbleNode[Map[K, V]]] = pairParser.sepBy(comma).asSeq.between(lParenCurl, rParenCurl).map(MAPNode.PARSER[K, V]())
      mapParser
    }
  ): _*)

  val mapParserAny: Parser[MapAbleNode[Map[Any, Any]]] = mapParser(MapAbleRef.`lazy`(), MapAbleRef.`lazy`())

  val mapAbleParser: Parser[MapAbleNode[Any]] =
    Parsers.or(Seq(
      BOOLEAN,
      idOrStringOrNrOrAnyOrAllParser,
      listParserAny,
      setParserAny.atomic, // atomic, as both Set and Map start with lParenCurl
      mapParserAny
    ): _*)
  ...
  val dataParser: Parser[DataNode] = kw(KW_DATA).next(mapParserAny).map(DataNode.PARSER)
  ...
  def parseProcessModels(reader: Reader): Set[ProcessNode] = {
    logger.debug("parseProcessModels START")

    // lex, parse & contextualise
    val _parser: Parser[Set[ProcessNode]] = processNodeParser.many().asSet.from(TOKENIZER, IGNORED)

    val processModels: Set[ProcessNode] = _parser.parse(reader)

    logger.debug("parseProcessModels END || ProcessNodes: " + processModels)
