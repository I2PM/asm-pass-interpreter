...
private object StructuralSoundnessAnalysis {
  ...
  private def analyzeProcess(p: Process): Unit = {
    val jobs: Seq[(Macro, String)] =
      p.subjects.collect[(InternalSubject, Set[Macro]), Set[(InternalSubject, Set[Macro])]] {
        case s: InternalSubject => (s, s.internalBehavior.additionalMacros + s.internalBehavior.mainMacro)
