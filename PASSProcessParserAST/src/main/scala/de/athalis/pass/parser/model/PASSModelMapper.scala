package de.athalis.pass.parser.model
...
object PASSModelMapper {
  ...
  private def toPASSSubject(sn: SubjectNode): Subject = {
    if (sn.isInterfaceSubject) {
      ...
      InternalSubject(identifier, internalBehavior, maximumInstanceRestriction, attributes)
    }
  }

  def toPASSProcess(p: ProcessNode): Process = {
