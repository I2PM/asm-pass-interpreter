...
object Model2ASMMap {
  type ProcessMap = Map[String, Any]
  ...
  def toMap(processes: Set[Process]): Map[ProcessIdentifier, ProcessMap] = {
    processes.map(p => {
      (p.identifier -> Model2ASMMap.toMap(p))
    }).toMap
  }

  def toMap(process: Process): ProcessMap = {
    ...
    val processAttributes: Set[ProcessAttribute] = process.attributes

    val subjects: Set[Subject] = process.subjects
    val startSubjectIDs: Set[SubjectIdentifier] = subjects.collect({
      case s: InternalSubject if s.attributes.contains(SubjectIsStartSubject) => s.identifier
    })

    val processMacros: Set[MacroWithInternalNumber] = process.macros.map(m => InternalNumberWrapped.construct[Macro, Option[Subject]](m, None))
    val macros: Set[MacroWithInternalNumber] = processMacros ++ subjects.collect{case s: InternalSubject => (s.internalBehavior.additionalMacros + s.internalBehavior.mainMacro).map(m => InternalNumberWrapped.construct[Macro, Option[Subject]](m, Some(s)))}.flatten
