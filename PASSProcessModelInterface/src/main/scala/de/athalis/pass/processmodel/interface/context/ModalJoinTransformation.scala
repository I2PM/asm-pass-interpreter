...
class ModalJoinAnalysis(val p: Process) extends Analysis[ModalJoinAnalysisResult] {
  ...
  override def analyze(): ModalJoinAnalysisResult = {
    ...
    results ++= p.subjects.collect({
      case s: FullySpecifiedSubject => (s.internalBehavior.mainMacro +: s.internalBehavior.additionalMacros.toSeq).map(m => ((Some(s.identifier), m.identifier) -> analyze(m)))
    }).flatten
